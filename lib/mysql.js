...
exports.findBlogByTag = (articleTag, page) => {
  let _sql = `select article_id, articleType, title, ellipsis, createtime, comments, reviews, articleTag, articleImg, blogs.author, likeCount, users.avatar from blogs inner join users on blogs.author = users.author and blogs.articleTag like "%${articleTag}%" order by createtime desc limit ${(page - 1) * 10}, 10;`
  return exec(_sql)
}
...
exports.getLikeByPage = (author) => {
  let _sql = `select blogs.article_id,articleType, createtime, articleTag, author, title,likeCount, articleImg, comments, reviews FROM blogs, likes where likes.like_author = '${author}' and likes.article_id = blogs.article_id order by likes.id desc `
  return exec(_sql)
}
...
// 获取通知文章
exports.getAbout = () => {
  let _sql = `select * from blogs where articleType = 'about';`
  return exec(_sql)
}
// 删除文章
...
exports.findBlogById = (id) => {
  let _sql = `select article_id, markdown,articleTag,articleType, title, createtime, content, author, articleImg, comments, reviews, likeCount from blogs where article_id = "${id}" `
  return exec(_sql)
}
...
//管理员登录
exports.managelogin = (username, password) => {
  console.log(username+password)
  let _sql = `select * from manage where username = "${username}" and password = "${password}";`
  return exec(_sql)
}


// 通过名字查找管理员
exports.findManage = (name) => {
  let _sql = `select * from manage where name like '%${name}%';`
  return exec(_sql)
}

//添加管理员
exports.addManage = (values) => {
  let _sql = `insert into manage set  name=?, username=?,password=? , avatar = ?, create_time = ?;`
  return exec(_sql, values)
}

//删除管理员
exports.deleteManage = (username) => {
  let _sql = `delete from manage where username='${username}';`
  return exec(_sql)
}

//获取管理员信息
exports.getManageInfo = (username) => {
  let _sql = `select * from manage where username = '${username}';`
  return exec(_sql)
}
...
// 分页查询用户
exports.findUserByPage = (top, page) => {
  let _sql = `select avatar, autograph, date, company, job, author from users limit ${(page - 1) * 10}, ${top};`
  return exec(_sql)
}

// 查询用户点赞数、评论、阅读总数
exports.findBlogbyUser = (author) => {
  let _sql = `select count(*) as blogs, sum(comments) as comments, sum(reviews) as reviews, sum(likeCount) as likes from blogs where author="${author}";`
  return exec(_sql)
}
...
// 查询用户关注总数
exports.findCountFollowingList = (author) => {
  let _sql = `select count(*) as follows from follows where following_author = '${author}';`
  return exec(_sql)
}
...
// 查询用户被关注总数
exports.findCountFollowList = (author) => {
  let _sql = `select count(*) as followings from follows where follow_author = '${author}';`
  return exec(_sql)
}
...
// 反馈意见
exports.createMessage = (values) => {
  let _sql = `insert into message set resource=?, content=?, phone=?, other=?;`
  return exec(_sql, values)
}
