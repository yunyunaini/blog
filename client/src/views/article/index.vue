...
export default class  extends Vue {
  ...
  private async changeArticle(articleId: any) {
    ...
    // const userInfo =  await getUserInfo({author: data.author})
    await (() => {this.loading = false})()
    data.article.createtime = formatTime(data.article.createtime)
    data.article.author = data.userInfo
    document.title = data.article.title // 设置页面的title
    const _toc: string[] = data.article.content.match(/<[hH][1-6]>.*?<\/[hH][1-6]>/g)
    if (_toc) {
      _toc.forEach((item: string, index: number) => {
        let _toc:string = `<div class='toc-title' id='${index}'>${item} </div>`
        data.article.content = data.article.content.replace(item, _toc)
      })
    }
